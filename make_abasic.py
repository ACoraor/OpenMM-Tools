...
def main():
    """Create a construct PDB by removing the nucleoside at the specified
    residue indices."""
    ...
    xyz, top = make_abasic(xyz, top, args.donor,
        args.acceptor)
...
def make_abasic(xyz, top, donor_ind, acceptor_ind):
    """Remove the appropriate atoms of the DNA residue to replace.
    ...
    Returns:
        abasic_xyz: *np.array*, shape: (n_atoms,3)
            Positions of particles in the labelled DNA pdb.
        abasic_top: *mdtraj.topology*
...
def remove_dna_atoms(xyz, top, donor_ind, acceptor_ind):
    ...
    Parameters:
        ...
        top: *mdtraj.topology*
            Topology of the input DNA pdb.
        donor_ind: *int*
            Index of the abasic site with respect to strand 1, 5' direction. If 
			negative, do not remove.
        acceptor_ind: *int*
            Index of the acceptor with respect to strand 1, 5' direction. If
			negative, do not remove.
    Returns:
    ...
    #Remove all atoms without an \' in the name which aren't in keeplist
    keep_names = set(['P','OP1','OP2'])
    ...
    if donor_ind >= 0:
        ...
        if donor_residue.name == "C5N" or donor_residue.name == "C3N":
            raise ValueError("Cannot make a cyanine dye abasic.")
        for at in donor_residue.atoms:
            has_no_quote = "'" not in at.name
            isnt_in_keep_names = at.name not in keep_names
            if has_no_quote and isnt_in_keep_names:
                rm_inds.append(at.index)
    ch2_res = [r for r in chains[1].residues]
    if acceptor_ind >= 0:
        acceptor_residue = ch2_res[acceptor_ind]
        if acceptor_residue.name == "C5N" or acceptor_residue.name == "C3N":
            raise ValueError("Cannot make a cyanine dye abasic.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d','--donor', type=int, 
        default=15, help='Index of abasic site in strand 1. Pass -1 for no donor. Default=15')
    parser.add_argument('-a','--acceptor', type=int, 
        default=23, help='Index of abasic site in strand 2. Pass -1 for no acceptor. Default=23 (B10)')
